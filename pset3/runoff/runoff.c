...
int find_min(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (!sorted_result[i].eliminated)
        {
            return sorted_result[i].votes;
        }
    }
    return 0;
}
...
{
    // TODO
    int remain_count = 0, remain_votes[candidate_count];
    for (int i = 0, j = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
            remain_votes[j] = candidates[i].votes;
            remain_count++;
            j++;
        }
    }
    ...
    {
        if (remain_votes[i - 1] != remain_votes[i])
        {
            return false;
        }
    }
